...
// ─── INDIAN CPCB NAQI CATEGORY & COLOR TOKEN MAPPER ─────────────────────────────
const getAqiCategoryInfo = (aqi) => {
  const val = Number(aqi) || 0;
  if (val <= 50)   return { label: "GOOD", color: "bg-green-600 text-white", hex: "#16a34a", bg: "rgba(22,163,74,0.14)", border: "rgba(22,163,74,0.35)", text: "#4ade80" };
  if (val <= 100)  return { label: "SATISFACTORY", color: "bg-lime-500 text-black", hex: "#84cc16", bg: "rgba(132,204,22,0.14)", border: "rgba(132,204,22,0.35)", text: "#a3e635" };
  if (val <= 200)  return { label: "MODERATE", color: "bg-yellow-400 text-black", hex: "#facc15", bg: "rgba(250,204,21,0.14)", border: "rgba(250,204,21,0.35)", text: "#facc15" };
  if (val <= 300)  return { label: "POOR", color: "bg-orange-500 text-white", hex: "#f97316", bg: "rgba(249,115,22,0.14)", border: "rgba(249,115,22,0.35)", text: "#fb923c" };
  if (val <= 400)  return { label: "VERY POOR", color: "bg-red-600 text-white", hex: "#dc2626", bg: "rgba(220,38,38,0.14)", border: "rgba(220,38,38,0.35)", text: "#f87171" };
  return { label: "SEVERE", color: "bg-red-950 text-red-200 border border-red-700", hex: "#450a0a", bg: "rgba(69,10,10,0.35)", border: "rgba(220,38,38,0.55)", text: "#fca5a5" };
};
...
export default function Screen4PollutionInvestigation() {
  ...
  const spikeLockedUntilRef = useRef(0);       // epoch ms — poller MUST NOT clear spike before this
  ...
  useEffect(() => {
    ...
    const fetchLive = async () => {
      ...
      try {
        ...
        // Only clear spike if the lock window has expired (2 min after trigger)
        if (spikeActiveRef.current && Date.now() > spikeLockedUntilRef.current) {
  ...
  const triggerSpike = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Guarantee the AQI pill reads 310 immediately by patching reading if backend
        // diurnal curve deflated it (mock source depends on time-of-day Gaussian).
        if (data?.trigger_station?.reading) {
          const existingAqi = data.trigger_station.reading.total_aqi ?? 0;
          if (existingAqi < 150) {
            data.trigger_station.reading.total_aqi = 310;
          }
        }
        // Force is_spike flag so guard clause sees it even before next poll
        data.is_spike = true;
        setDashboardData(data);
        ...
        <div className="sidebar-scroll">
          ...
          <div className="header-row">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
              ...
              {(() => {
                const aqiVal = Math.round(currentStationObj.aqi);
                const cat = getAqiCategoryInfo(aqiVal);
                return (
                  <div
                    className={`aqi-pill ${cat.color}`}
                    title={`CPCB NAQI ${aqiVal} - ${cat.label}`}
                    style={{
                      background: cat.bg,
                      borderColor: cat.border,
                    }}
                  >
                    <div className="aqi-pulse-dot" style={{ background: cat.hex, boxShadow: `0 0 8px ${cat.hex}` }}/>
                    <div>
                      <div className="aqi-number" style={{ color: cat.text }}>
                        {aqiVal > 0 ? aqiVal : '\u2014'}
                      </div>
                      <div className="aqi-label" style={{ fontSize: 9, fontWeight: 800, color: cat.text, letterSpacing: '0.05em' }}>
                        {cat.label} (CPCB)
                      </div>
                    </div>
                  </div>
          ...
          />
            ...
            {/* ── FORENSIC ACTION CENTER ──
                Guard: render when AQI > 150 AND is_spike, OR when spikeActive is set
                regardless of AQI (simulation always shows FAC). */}
            {(currentStationObj.is_spike || spikeActive) && (
              <div
              ...
              >
                <div
                  className="flex items-center justify-between border-b border-red-500/20 pb-2"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <h4
                    className="text-xs font-bold uppercase tracking-wider text-red-400 flex items-center gap-1"
                    style={{
                      fontSize: 11,
                      fontWeight: 800,
                  ...
                  >
                    {activeData?.actionable_intelligence?.squad_id || "PMC-AQ-SQUAD-01"}
                  </span>
                </div>

                <div
                  className="grid grid-cols-2 gap-2 text-xs font-mono text-slate-300"
                  style={{
                    display: 'grid',

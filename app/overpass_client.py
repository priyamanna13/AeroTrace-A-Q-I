...
# ---------------------------------------------------------------------------
# Module-level discovery cache — prevents re-running Overpass network
# queries after the first successful discovery. Keyed by bbox string.
# ---------------------------------------------------------------------------
...
class OverpassSourceDiscovery:
    ...
    def discover_sources(self) -> list[dict]:
        ...
        """
        bbox_key = self._format_bbox()

        # ── Cache gate: skip all network I/O if already discovered ──
        if bbox_key in _DISCOVERY_CACHE:
            logger.debug("Overpass cache hit for bbox %s — returning %d cached sources",
                         bbox_key, len(_DISCOVERY_CACHE[bbox_key]))
            return _DISCOVERY_CACHE[bbox_key]
...
def discover_and_format(city_config: dict) -> list[dict]:
    ...
    runs all queries. Results are cached at the discovery layer — repeated
    calls with the same city config return in < 1ms.
